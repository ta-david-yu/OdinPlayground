...
SetContextAsGlobal :: proc(guiContext: ^GUIContext) {
	g_Context = guiContext
}

FreeContext :: proc(guiContext: ^GUIContext) {
	if g_Context == guiContext {
		g_Context = nil
	}
	delete(guiContext.Style.FontConfigStack)
	delete(guiContext.State.Frame.DrawCommands)
	free(guiContext)
}

SetMeasureTextFunction :: proc(
	func: proc(textContent: string, fontConfig: FontConfig, userData: rawptr) -> Dimensions,
	userData: rawptr,
) {
	g_Context.Functions.MeasureText = func
	g_Context.Functions.MeasureTextUserData = userData
}
...
NewFrame :: proc() {
	...
	// Clear active states
	if (state.ActiveId != 0 &&
		   state.ActiveIdIsAlive != state.ActiveId &&
...
Button :: proc(label: string, position: [2]f32) -> bool {
	...
	if (NextItemDataFlag.HasSize in guiContext.NextItemData.Flags) {
		...
		fullRectWidth :=
			useFixedWidth ? guiContext.NextItemData.Size.x : textDimensions.x + xPadding
		fullRectHeight :=
			useFixedHeight ? guiContext.NextItemData.Size.y : textDimensions.y + yPadding
		fullRect = Rect {
			Position = position,
			Size     = {fullRectWidth, fullRectHeight},
		}

		// At the moment we align text to the center middle.
		xTextRect :=
			position.x +
			(useFixedWidth ? fullRect.Size.x * 0.5 - textDimensions.x * 0.5 : style.Variables.Button.FramePaddingLeft)
		yTextRect :=
			position.y +
			(useFixedHeight ? fullRect.Size.y * 0.5 - textDimensions.y * 0.5 : style.Variables.Button.FramePaddingTop)
		textRect = Rect {
			Position = {xTextRect, yTextRect},
		...
		fullRect = Rect {
			Position = position,
			Size     = textDimensions.xy + {xPadding, yPadding},
		}
		textRect = Rect {
			Position = position + {
					style.Variables.Button.FramePaddingLeft,
					style.Variables.Button.FramePaddingTop,
				},
			Size     = textDimensions.xy,
		}
	}
	...
	if (style.Variables.Button.InnerBorderThickness == 0 &&
		   style.Variables.Button.OuterBorderThickness == 0) {
...
Text :: proc(label: string, position: [2]f32) {
	...
	if (NextItemDataFlag.HasSize in guiContext.NextItemData.Flags) {
		...
		// At the moment we align text to the center middle.
		xTextRect :=
			position.x + (useFixedWidth ? fullRect.Size.x * 0.5 - textDimensions.x * 0.5 : 0)
		yTextRect :=
			position.y + (useFixedHeight ? fullRect.Size.y * 0.5 - textDimensions.y * 0.5 : 0)
		textRect = Rect {
			Position = {xTextRect, yTextRect},
			Size     = textDimensions.xy,
		}
