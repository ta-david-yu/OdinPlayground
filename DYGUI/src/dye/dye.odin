...
Init :: proc(windowSettings: WindowSettings)
{
    g_Memory = new (EngineMemory)
    g_Memory.MainWindowSettings = windowSettings
}

SetGameFunctions :: proc(functions: GameFunctions)
{
    if g_Memory == nil 
    {
        sdl3.Log("Engine memory is not allocated. You might have forgot to call `Init` first.");
        return
    }

    g_Memory.GameFunctions = functions
}
...
StartLoop :: proc() 
{
    ...
    windowSettings := g_Memory.MainWindowSettings
    g_Memory.MainWindow = sdl3.CreateWindow(windowSettings.Name, windowSettings.Width, windowSettings.Height, sdl3.WINDOW_RESIZABLE)
    if (g_Memory.MainWindow == nil)
    {
        sdl3.Log(sdl3.GetError())
        return
    }
    defer sdl3.DestroyWindow(g_Memory.MainWindow)

    rendererDriverName : cstring = ""
    g_Memory.MainRenderer = sdl3.CreateRenderer(g_Memory.MainWindow, rendererDriverName)
    if (g_Memory.MainRenderer == nil)
    {
        sdl3.Log(sdl3.GetError())
        return
    }
    defer sdl3.DestroyRenderer(g_Memory.MainRenderer)
    sdl3.SetRenderLogicalPresentation(g_Memory.MainRenderer, windowSettings.Width, windowSettings.Height, sdl3.RendererLogicalPresentation.LETTERBOX)
    ...
    g_Memory.TextEngine = ttf.CreateRendererTextEngine(g_Memory.MainRenderer)
    if (g_Memory.TextEngine == nil)
    {
        sdl3.Log(sdl3.GetError())
        return
    }
    defer ttf.DestroyRendererTextEngine(g_Memory.TextEngine)

	dygui.Init(dygui.Canvas{Width=cast(f32)g_Memory.MainWindowSettings.Width, Height=cast(f32)g_Memory.MainWindowSettings.Height})
	dygui.SetMeasureTextFunction(measureText)

	g_Memory.GameFunctions.OnBeforeEngineStarts(true)
    ...
    for
    {
        deltaTimeInMiliseconds := time - timeLastFrame
		...
		for sdl3.PollEvent(&event) 
		{
			#partial switch event.type 
			{
				...
				case .MOUSE_MOTION:
					...
					if sdl3.GetRenderLogicalPresentationRect(g_Memory.MainRenderer, &rendererRect)
					{
						x -= rendererRect.x
						y -= rendererRect.y
        ...
        // Update
        g_Memory.GameFunctions.OnUpdate(deltaTimeInSeconds)

        // GUI
		dygui.NewFrame()
		{
            g_Memory.GameFunctions.OnImGui(deltaTimeInSeconds)
		}
		dygui.EndFrame()

        // Render
		sdl3.SetRenderDrawColor(g_Memory.MainRenderer, 180, 180, 180, sdl3.ALPHA_OPAQUE)
		sdl3.RenderClear(g_Memory.MainRenderer)

        g_Memory.GameFunctions.OnRender(deltaTimeInSeconds)
        renderImGuiFrame()

        sdl3.RenderPresent(g_Memory.MainRenderer)
...
@(private)
renderImGuiFrame :: proc()
{
    renderer := g_Memory.MainRenderer
    fonts := g_Memory.Fonts
    textEngine := g_Memory.TextEngine
...
Shutdown :: proc()
{
    for i := 0; i < len(g_Memory.Fonts); i += 1
    {
        ttf.CloseFont(g_Memory.Fonts[i])
    }

    free(g_Memory)
}

LoadFont :: proc(fontPath: cstring, defaultSize: f32) -> int
{
    ...
    append(&g_Memory.Fonts, font)
    return len(g_Memory.Fonts) - 1
}

@(private)
measureText :: proc(textContent: string, fontConfig: dygui.FontConfig) -> dygui.Dimensions 
{
	font := g_Memory.Fonts[fontConfig.FontId]
	currentFontSize := ttf.GetFontSize(font)
